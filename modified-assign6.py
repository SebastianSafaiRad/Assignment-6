    ...
    def buildGraph2(self):
        ...
        while self.q.size() > 0:
            ...
            if not self.container_a.isFull():
                # the code here is explicit, step-by-step to show what's being done
                # it's been condensed/reduced/simplified in the following "if" blocks
                self.container_a.fill()
                newStateString = self.getStateString()
                newVertex = self.addEdge(parentStateString, newStateString)
                self.addToQueue(newVertex)
                self.resetContainers(parentStateString)
